...
特别说明：
脚本运行必须填写种子token，iOS用户使用代理可以直接获取；Android用户需要抓包获取种子token，手动做京喜农场任意任务即可获取种子token，推荐使用elecV2P（使用设置类似iOS用户的代理软件）或者HttpCanary，搜索关键字"farm_jstoken"，token按照{"farm_jstoken":"xxx","timestamp":"xxx","phoneid":"xxx-xx x"}格式填写即可
...
function getAssistUser() {
    return new Promise(resolve => {
        try {
            $.get({url: `https://raw.githubusercontent.com/shuye72/RandomShareCode/master/JX_Fruit.json`, timeout: 10000}, async (err, resp, _data) => {
                try {
                    const {code, data = {}} = JSON.parse(_data);
                    if (data.value) {
