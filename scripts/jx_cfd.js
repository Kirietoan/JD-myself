...
function createSuperAssistUser() {
  return new Promise(resolve => {
    ...
    $.get({ url: 'https://raw.githubusercontent.com/shuye72/RandomShareCode/master/JX_CFD.json' }, async (err, resp, _data) => {
      try {
...
function joinGroup() {
  return new Promise( async (resolve) => {
    $.get({ url: 'https://raw.githubusercontent.com/shuye72/RandomShareCode/master/JX_CFD_Group.json' }, (err, resp, _data) => {
      try {
        const { data = {} } = JSON.parse(_data);
