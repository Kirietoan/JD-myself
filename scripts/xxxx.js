...
一次性活动，运行完脚本获??53京豆，进入入口还可以开卡领30??
*/
const $ = new Env('开卡活??');
            ...
            if(data['success']){
              for (let i = 0; i < data.data.myTasks.length; ++i){
                ...
                if(task.groupType===1) {
                  ...
                  $.log(`关注店铺任务，完成次??: ${task.finishCount}/${task.itemCount}`)
                } else if(task.groupType===2){
                  groupType = 'ADD_CART'
                  $.log(`加购任务，完成次??: ${task.finishCount}/${task.itemCount}`)
                  ...
                  $.log(`浏览商品任务，完成次??: ${task.finishCount}/${task.itemCount}`)
                }
              ...
              if (data.data.rewardStatus!==3) {
                $.log(`去领取额??${data.data.rewardFinish}京豆`)
                await doTask("FINISH_TASK",0)
                const res = await doTask("FINISH_TASK",0)
                if(res['success']){
                  $.log(`领取额外${data.data.rewardFinish}京豆完成成功，获??${data.data.rewardFinish}京豆`)
                }
              }
            } else{
              console.log('获取失败，请检查是否黑??')
            }
          }
        }
      } catch (e) {
...
function taskUrl(functionId,body) {
  return {
    ...
    headers: {
      ...
      'User-Agent': $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : "jdapp;iPhone;9.2.2;14.2;京东/9.2.2 CFNetwork/1206 Darwin/20.1.0") : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.2.2;14.2;京东/9.2.2 CFNetwork/1206 Darwin/20.1.0"),
